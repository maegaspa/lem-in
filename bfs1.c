...
{
    //printf("bfs->queue[%d][%d] = %d\n", temp.actual_path, temp.i_queue + 1, bfs->queue[temp.actual_path][temp.i_queue + 1]);
    //printf("temp.actual_path = %d\n", temp.actual_path);

	bfs->mtx_state[bfs->queue[temp.actual_path][temp.i_queue]][temp.actual_path] = step;
	printf("bfs->mtx_state[queue = %d][%d] = %d\n", bfs->queue[temp.actual_path][temp.i_queue], temp.actual_path, bfs->mtx_state[bfs->queue[temp.actual_path][temp.i_queue]][temp.actual_path]);
}
...
void            ft_foundpaths(t_bfs *bfs, int step)
{
    ...
    while (++temp.actual_path < bfs->start_paths)
    {
        ...
        {
            //printf("bfs->queue[%d][%d] = %d\n", temp.actual_path, temp.i_queue, bfs->queue[temp.actual_path][temp.i_queue]);
...
//    j = 0;
    ...
    ft_foundpaths(&bfs, step);
    printf("HUGREBOU--3\n");
   // print_tab_int(bfs.mtx_state, i, j);
    return (0);
}
